...
KEYS = (
  [(os.getenv('OPENAI_API_KEY'), os.getenv('OPENAI_ORGANIZATION'))] + [(os.getenv(f'OPENAI_API_KEY2'), os.getenv(f'OPENAI_ORGANIZATION'))]
...
def prepare_dataset(args, skip_embs = False):
  ...
  if args.prompt_mode == 'retrieval' and not skip_embs and args.get('tr_questions', None) is None:
    split = args.split
    ...
    if os.path.exists(f'embeddings/{dataset_string}_cot.npy'):
      ...
      args.graham_matrix = a.dot(a.T) * (1. - np.eye(len(a)))
  ...
  if dataset_string == 'gsm8k':
    ...
    if args.split == 'train':
      ...
      idxs = rng.choice(len(dataset), size=min(len(dataset), args.size_limit), replace=False)
      dataset = dataset[idxs]
    QUESTIONS = [f"Q: {q}\nA:" for q in dataset['question']]
    ANSWERS = [pred_cleansing(args, a.split('####')[-1]).strip() for a in dataset['answer']]
    ...
    for subject in {'abstract_algebra',
      'anatomy',
      'astronomy',
      'business_ethics',
      'clinical_knowledge',
    ...
    prepare_dataset.groups = {}
    for subject in ['Prealgebra', 'Algebra', 'Intermediate Algebra', 'Precalculus', 'Geometry', 'Counting & Probability', 'Number Theory']:
    ...
    if args.split == 'train':
      ...
      idxs = rng.choice(len(dataset), size=min(len(dataset), args.size_limit), replace=False)
      dataset = dataset[idxs]
    amod = lambda a: ' '.join(a).replace('A)', '(a) ').replace('B)', '(b) ').replace('C)', '(c) ').replace('D)', '(d) ').replace('E)', '(e) ')
    QUESTIONS = [f"Q: {q} Answer Choices: {amod(a)}\nA:" for q, a in zip(dataset['question'], dataset['options'])]
    ANSWERS = [a.lower() for a in dataset['correct']]

  elif dataset_string == 'aqua_direct':
    dataset = load_dataset("aqua_rat", split='test')
    amod = lambda a: '{' + ', '.join(a).replace('A)', '').replace('B)', '').replace('C)', '').replace('D)', '').replace('E)', '') + '}'
...
def make_mixed_dataset(args, few_shot=None):
  ...
  if isinstance(few_shot, str):
    questions = np.array([f"{few_shot}\n\n{q}" for q in questions[:args.size_limit]])
  else:
    questions = np.array([f"{few_shot[i]}\n\n{q}" for i, q in enumerate(questions[:args.size_limit])])
  ...
  for i in range(5): # try 5 times, otherwise just return ''
    try:
      if ARGS.prompt_mode == 'self_prompted':
        o = decoder.decode(args, q, args.sample_len - i*40, KEYS[j % len(KEYS)], args.samples, temp=0.7)
      else:
        o = decoder.decode(args, q, args.sample_len, KEYS[j % len(KEYS)], args.samples, temp=0.7, stop=['Q:', '\n\n'])
      write_log('.',end='')
...
def get_preds(args, qs, decoder):
  ...
  while len(outputs) < len(qs):
    ...
    q = qs[len(outputs)]
    try:
      o = decoder.decode(args, q, args.sample_len, KEYS[i % len(KEYS)], args.samples, temp=0.7, stop=['Q:', '\n\n'])
      outputs.append(o)
    except openai.error.RateLimitError as e:
      write_log(i, 'LIMIT',end=',')
      time.sleep(args.api_error_delay)
...
def extract_pred(pred):
  ...
  if ARGS.dataset == 'aqua_direct':
    ...
    e = '.'.join(pred.split('answer is')[-1].strip().split('.')[:-1]).replace('.00', '.0')
    try:
      return str(float(e))
    except ValueError:
      return ''
...
def evaluate(preds, answers=None, predict_true_answer=False, weights=None):
  ...
  for i, (p, w) in enumerate(zip(all_ps,weights)):
    if predict_true_answer:
      ...
      for p_, w_ in zip(p, w):
        c_[p_] += w_ # NOTE: Counter doesn't officially support floats, but it seems to work
          
      c = list(filter(lambda x: x[0], c_.most_common(3))) # filter out non-answers
      if len(c) > 1: 
        ps.append(c[0][0])
  ...
  if ARGS.cot_mode == 'complexity':
    len_cots = [[len(c.replace('\n','. ').split('. ')) for c in cot] for cot in preds]
    sample_cots = []
    for cots, i, lc in zip(preds, idxs, len_cots):
      try:
        cots = np.array(cots)[i]
        lc = np.array(lc)[i]
        sample_cots.append(cots[np.random.choice(np.argsort(lc)[-5:])]) # choose one of the longest 5 
      except:
        sample_cots.append('')
  elif ARGS.cot_mode == 'random':
    sample_cots = [(cots[np.random.choice(i)] if len(i) else '') for cots, i in zip(preds, idxs)]
  else:
    raise
...
def get_new_prompt(args, questions, preds, answers):
  ...
  if args.prompt_mode=='retrieval':
    ...
    for j in range(len(questions)):
      ...
      new_few_shots = [(f"Q:{q.split('Q:')[-1]}{a}") for q,a in zip(args.tr_questions[nearest_idxs], args.tr_answers[nearest_idxs])]
      res.append('\n\n'.join(new_few_shots))
    write_log("**************************")
  ...
  if args.prompt_mode=='nearest_neighbor_suitable':
    ...
    for j in range(len(questions)):
      nearest_idxs = np.random.choice(sa_idxs[nearest_suitable[j]], min(8, len(nearest_suitable[j])), replace=False)
      new_few_shots = [(f"Q:{q.split('Q:')[-1]}{a}") for q,a in zip(np.array(questions)[nearest_idxs], self_generated_answers[nearest_idxs])]
    ...
    for j in range(len(questions)):
      nearest_idxs = np.random.choice(nearest[j], min(8, len(nearest[j])), replace=False)
      new_few_shots = [(f"Q:{q.split('Q:')[-1]}{a}") for q,a in zip(np.array(questions)[nearest_idxs], self_generated_answers[nearest_idxs])]
      accs.append(correct_answers[nearest_idxs].mean())
      res.append('\n\n'.join(new_few_shots))
    write_log("Average Prompt Accuracies:", np.mean(accs))
    ...
    for j in range(len(questions)):
      ...
      random_idxs = np.random.choice(len(jpreds), min(5, len(jpreds)), replace=False)
      new_few_shots = [f"A:{jpreds[i]}" for i in random_idxs]
      res.append(f"Q:{questions[j].split('Q:')[-1].split('A:')[0]}\n" + '\n\n'.join(new_few_shots))
    ...
    suitable_agreement[np.argsort(agreement)[:min(24, (agreement >= min_agreement).sum())]] = True

  elif args.prompt_mode=='boosted':
    confs = evaluate(preds, answers, predict_true_answer=True)[0] * np.logical_not(correct_answers)
    og = (confs > 0).sum() # count of incorrect overall answers with at least 1 correct CoT
    ...
    if args.boosting_priority=='random':
      ...
      suitable_agreement = np.zeros_like(confs, dtype=bool)
      suitable_agreement[np.random.choice(len(suitable_agreement), 8, replace=False)] = True
    else:
      raise
    
    self_generated_answers = np.array(evaluate(preds, answers, predict_true_answer=True)[3])
    # but we need to regenerate the self generated answers... .

  else:
    raise NotImplementedError

  idxs = np.random.choice(np.where(suitable_agreement)[0], min(8, suitable_agreement.sum()), replace=False)
  write_log(f"New prompt size: {len(idxs)} / {suitable_agreement.sum()} ({i})")

  write_log("New prompt accuracy:", evaluate(preds[idxs], answers[idxs])[1], evaluate(preds[idxs], answers[idxs])[2])
  write_log("Answers:", answers[idxs])
  if args.prompt_mode=='boosted':
    write_log("BOOSTED prompt confidences:", evaluate(preds[idxs], answers[idxs], predict_true_answer=True)[2])
  new_few_shots = [(f"Q:{q.split('Q:')[-1]}{a}") for q,a in zip(np.array(questions)[idxs], self_generated_answers[idxs])]
  new_prompt = '\n\n'.join(new_few_shots)
  return new_prompt
...
def eval_by_group(preds, answers, groups):
  ...
  for i, idxs in enumerate(groups):
    write_log(evaluate(np.array(preds)[idxs], np.array(answers)[idxs])[2],
     '-', np.round(evaluate(np.array(preds)[idxs], np.array(answers)[idxs])[0].mean(), 2), f"- {i} ({len(idxs)})")
...
def main(**kwargs):
  ...
  elif os.path.exists(os.path.join('preds', f"{ARGS.dataset}_{ARGS.seed}.pickle")) and (ARGS.split == 'test') and (ARGS.prompt_mode!='retrieval') and not ARGS.complexity_prompting and not ARGS.prompt_tag:
    with open(os.path.join('preds', f"{ARGS.dataset}_{ARGS.seed}.pickle"), 'rb') as f:
      ps = pickle.load(f)
  ...
  for i in range(k, k + ARGS.iters):
    ...
    if len(ps):
      if ARGS.prompt_mode == 'boosted' or sum(map(lambda x: len(x[0]), ps)) < 10:
        ...
        not_idxs = np.where(evaluate(preds, answers)[0] >= ARGS.min_agreement)[0]
      idxs_.append(idxs)
