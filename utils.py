...
def decoder_for_gpt3(args, input, max_length, i, num_generations, temp, stop, echo, get_logits=False):
        
    # https://beta.openai.com/account/api-keys
    openai.api_key = os.getenv("OPENAI_API_KEY")
    openai.organization = os.getenv("OPENAI_ORGANIZATION")
    if isinstance(i, str):
      openai.organization = i
    elif isinstance(i, list) or isinstance(i, tuple):
      openai.api_key = i[0]
      openai.organization = i[1]
    #print(openai.api_key)

    engine = get_engine(args)
    if max_length == -1:
      max_length = 4064 - len(gpt2_tokenizer.tokenize(input))
    ...
    response = openai.Completion.create(
      ...
      stop=stop,
      echo=echo,
      logprobs=1 if get_logits else 0
    )
    
    if get_logits:
      if num_generations > 1 or isinstance(input, list):
        return [response["choices"][j]["text"] for j in range(len(response["choices"]))],\
                [response["choices"][j]["logprobs"]["token_logprobs"] for j in range(len(response["choices"]))]
...
def decoder_for_banana(args, input, max_length, i, num_generations, temp, stop, echo, get_logits=False):
    if get_logits:
        raise NotImplementedError

    

# Sentence Generator (Decoder) for Cohere ...
def decoder_for_cohere(args, input, max_length, i, num_generations, temp, stop, echo, get_logits=False):
    if get_logits:
        raise NotImplementedError

    assert args.model in ('medium', 'xlarge')

    client = cohere.Client(os.getenv('COHERE_API_KEY'))

    if max_length == -1:
        max_length = 2048 - len(client.tokenize(input).token_strings)
        if max_length < 0:
            raise Exception("Prompt is too long. Cohere's max prompt + prediction is 2048 tokens.")
    ...
    if num_generations > 1 or isinstance(input, list):
        return [g.text for g in response.generations]
...
class Decoder():
    ...
    def decode(self, args, input, max_length, i=None, num_generations=1, temp=0, stop=None, echo=False, get_logits=False):
        t = time.time()

        if args.api == 'openai':
            response = decoder_for_gpt3(args, input, max_length, i, num_generations, temp, stop, echo, get_logits)
        elif args.api == 'cohere':
            assert not echo
            assert stop is None
            response = decoder_for_cohere(args, input, max_length, i, num_generations, temp)
        elif args.api == 'banana':
            assert not echo
            assert stop is None
        else:
            raise Exception("Invalid api.")
        
        time.sleep(max(args.api_time_interval - (time.time() - t), 0))
...
def pred_cleansing(args, pred):
    if '=' in pred:
        pred = pred.split('=')[-1].strip()
    return answer_cleansing(args, pred, verbose=False)

# ver 0.2
def answer_cleansing(args, pred, verbose=True):

    if verbose:
      print("*")
      print("pred_before : " + pred)

    if args.method in ("few_shot", "few_shot_cot", "few_shot_majority", "few_shot_cot_majority"):
    ...
    if len(pred) == 0:
        ...
        if args.method in ("few_shot", "few_shot_cot", "few_shot_majority", "few_shot_cot_majority"):
            if answer_flag:
                ...
                pred = pred[-1]
        elif args.method in ("zero_shot", "zero_shot_cot", "consensus", "consensus2x", "zero_shot_majority", "zero_shot_cot_majority"):
            # choose the first element in list ...
            pred = pred[0]
        else:
...
def create_demo_text(args, cot_flag, stop_seq_flag):
    x, z, y = [], [], []
