...
def decoder_for_gpt3(
    args, input, max_length, i, num_generations, temp, stop, echo, get_logits=False
):
    ...
    if get_logits:
        if num_generations > 1 or isinstance(input, list):
            return [
                response["choices"][j]["text"] for j in range(len(response["choices"]))
            ], [
                response["choices"][j]["logprobs"]["token_logprobs"]
...
def decoder_for_cohere(
    ...
    if max_length == -1:
        ...
        if max_length < 0:
            raise Exception(
                "Prompt is too long. Cohere's max prompt + prediction is 2048 tokens."
            )
...
class Decoder:
    ...
    def decode(
        ...
        if args.api == "openai":
            ...
            assert not echo
            assert stop is None
            response = decoder_for_cohere(
                args, input, max_length, i, num_generations, temp
            )
